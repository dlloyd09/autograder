...
[start=3]
. Execute `./setup.sh`
. Ensure that *each* student's submission is properly formatted before running this script. This might require renaming files in student submissions after checking out the `pset6` branch. In particular, a student's submission must be organized as follows:
